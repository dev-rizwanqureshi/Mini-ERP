...
const props = defineProps<{ customers: any[]; products: any[] }>();
...
const form = useForm({
  ...
  terms: "Net 30",
  footer: "",
  items: [{ product_id: "", description: "", quantity: 1, unit_price: 0, tax_rate: 0, discount_type: "fixed", discount_value: 0 }],
});
...
<template>
  <form class="grid gap-6 p-4 md:grid-cols-[1fr_320px] md:p-6" @submit.prevent="form.post('/invoices')">
    <div class="space-y-5">
    ...
    <aside class="space-y-4 rounded-md border p-5 dark:border-neutral-800">
      ...
      <BaseButton type="submit" class="w-full" :loading="form.processing">Save Invoice</BaseButton>
    </aside>
  </form>
</template>
